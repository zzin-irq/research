  ...
  return (
    <>
      ...
      <div className="max-w-content mx-auto px-4 md:px-12 py-16">
        ...
        {isLoading && <p className="text-text-muted">جاري تحميل المواضيع...</p>}
        {error && <p className="text-red-600">حدث خطأ أثناء تحميل المواضيع.</p>}

        {data && data.items && (
          <div className="grid gap-6 md:grid-cols-2">
            {data.items.map((topic: any) => (
              <Link 
                key={topic.id} 
              ...
              >
                <h3 className="font-serif text-2xl group-hover:text-primary mb-3 text-text">
                  {topic.name}
                </h3>
